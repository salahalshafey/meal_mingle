            ...
            patternMatcher(
              ...
              if (inlineString.type == StringTypes.bulleted) {
                ...
                if (inlineString.string.startsWith(
                  RegExp(r'^(\s*[-*+]\s+)', multiLine: true, dotAll: false),
                )) {
                  return BulletedList(
                    textDirection: getDirectionalityOf(data),
                ...
                return BulletedList(
                  ...
                  bullet: Text(
                    inlineString.string.firstMatch(
                      RegExp(r'\d+\.', multiLine: true, dotAll: false),
                    ),
...
class _TextWellFormattedWitouthBulletedState
      ...
      children:
          patternMatcher(
            ...
            if (inlineText.type == StringTypes.bold) {
              return TextSpan(
                text: inlineText.string.substring(
                  2,
                  inlineText.string.length - 2,
                ),
                style: TextStyle(
                  fontSize: widget.fontSize?.plusOne ?? 15,
  ...
  Widget build(BuildContext context) {
    return widget.isSelectableText
        ...
        : Text.rich(_getTextSpan(context), textDirection: widget.textDirection);
  }
}
  ...
  Widget build(BuildContext context) {
    return Table(
      ...
      children: getRows.map((row) {
        return TableRow(
          children: getCells(row).map((cell) {
            return TableCell(
              child: Padding(
